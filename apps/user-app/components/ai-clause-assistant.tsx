    ...
    if (embedded) {
        return (
            <div className="flex flex-col h-full bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                {/* Header */}
                <div className="bg-violet-50/50 border-b border-violet-100 px-4 py-3 flex items-center gap-3">
        ...
        <div className="fixed right-4 top-24 w-96 max-h-[calc(100vh-120px)] bg-white rounded-2xl shadow-2xl border border-slate-200 overflow-hidden z-50 flex flex-col animate-in slide-in-from-right-5 fade-in duration-300">
            <div className="bg-slate-900 text-white px-4 py-3 flex items-center justify-between shadow-sm">
                <div className="flex items-center gap-2">
                    <MaterialIcon name="auto_awesome" size={16} className="text-violet-400" />
            ...
            <div className="flex border-b border-slate-100 bg-slate-50/50">
                ...
                >
                    <div className="flex items-center justify-center gap-2">
                        <MaterialIcon name="auto_fix_high" size={14} /> Generator
                    </div>
                </button>
                <button
                ...
                >
                    <div className="flex items-center justify-center gap-2">
                        <MaterialIcon name="description" size={14} /> Review
                    </div>
                </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-slate-50/30">
                {mode === 'suggest' ? (
                    <div className="space-y-4">
                        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm transition-all focus-within:ring-2 focus-within:ring-violet-100">
                            ...
                            <div className="space-y-4">
                                <div>
                                    ...
                                    <div className="relative">
                                        <select
                                            ...
                                            className="w-full pl-3 pr-8 py-2.5 border border-slate-200 rounded-lg text-sm bg-slate-50 text-slate-700 focus:border-violet-500 focus:ring-1 focus:ring-violet-500 outline-none appearance-none"
                                        >
                                ...
                                <div>
                                    ...
                                    <textarea
                                        ...
                                        className="w-full px-3 py-2.5 border border-slate-200 rounded-lg text-sm bg-slate-50 resize-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 outline-none placeholder:text-slate-400"
                                        rows={3}
                                    />
                                </div>
                            </div>
                            ...
                            >
                                {isLoading ? (
                                    <span className="flex items-center gap-2"><MaterialIcon name="auto_awesome" size={14} className="animate-spin" /> Thinking...</span>
                                ) : (
                                    <span className="flex items-center gap-2"><MaterialIcon name="auto_awesome" size={14} /> Generate Clause</span>
                                )}
                            </Button>
                        </div>
                        ...
                        {suggestions.length > 0 && (
                            <div className="space-y-3 animate-in fade-in slide-in-from-bottom-2 pb-4">
                                ...
                                {suggestions.map((s, i) => (
                                    <div key={i} className="bg-white p-3.5 rounded-xl border border-slate-200 shadow-sm hover:border-violet-300 hover:shadow-md transition-all group">
                                        ...
                                        <Button size="sm" variant="outline" className="w-full text-xs h-8 border-slate-200 text-slate-600 hover:border-violet-500 hover:text-violet-600 hover:bg-violet-50" onClick={() => handleInsert(s.content)}>
                                            Insert Clause
                                        </Button>
                                    </div>
                                ))}
                    ...
                    <div className="space-y-4">
                        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                            ...
                            <div className="bg-slate-50 p-3 rounded-lg border border-slate-100 text-xs text-slate-600 min-h-[80px] mb-4 overflow-y-auto max-h-[150px] font-mono leading-relaxed">
                                {selectedText || <span className="text-slate-400 italic flex items-center justify-center h-full gap-2"><MaterialIcon name="description" size={14} /> No text selected...</span>}
                            </div>
                            <Button
                                onClick={handleImproveClause}
                                className="w-full bg-violet-600 hover:bg-violet-700 text-white shadow-lg shadow-violet-600/10"
            ...
            <div className="p-3 bg-white border-t border-slate-200 relative z-10">
                <div className="relative">
                    <input
                        ...
                        className="w-full pl-4 pr-10 py-2.5 rounded-full border border-slate-200 focus:outline-none focus:border-violet-500 focus:ring-1 focus:ring-violet-500 text-sm bg-slate-50 transition-all placeholder:text-slate-400"
                    />
                    <button className="absolute right-1.5 top-1.5 p-1.5 bg-slate-200 rounded-full text-slate-500 hover:bg-violet-600 hover:text-white transition-all">
